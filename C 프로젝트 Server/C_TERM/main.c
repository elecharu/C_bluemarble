...
void main()
{
	...
	system("mode con cols=50 lines=7 | title 부르마블 Server");
	...
	sin.sin_port = htons(port);						// 서버 포트번호 설정
	...
	s = accept(s, (SOCKADDR*)&cli_addr, &cli_size);
	...
	//char send_x[BUFSIZE];
	//recv(cs, message, 1024, 0); // 데이터 수신 , recv(클라이언트소켓핸들,데이터 받아올 변수,수신할 데이터 최대 길이, 데이터 수신 기호);
	...
	char user_buf[BUFSIZE + 1];
	char bal_buf[BUFSIZE + 1];
	...
	while (1)
	{
		...
		case INTRO:
		{
			...
			intro_size();
			intro_status = draw_intro(color);
			if (intro_status == START)
				STATE = INIT;
			else if (intro_status == EXIT) {
				STATE = EXIT_GAME;
				White1;
			}
			else
		...
		case INIT:
		{
			...
			user_set = 2;			//게임 인원수 설정 부분
			gameover_cnt = user_set;

			//-----------------------------------------------------------------------------------------------
			_itoa(user_set, user_buf, 10);
			send(s, user_buf, strlen(user_buf), 0);// 서버에서 클라이언트로 게임 인원수 user_set 부분을 보낸다.
			//-----------------------------------------------------------------------------------------------

			White1;
			while ((bal > 2000) | (bal < 500)) {
				gotoxy(15, 15); //gotoxy(50, 24);
				printf("초기 보유금액을 설정해주세요(단위는 만원이며, 최대 2000만원입니다.) : ");
				scanf_s("%d", &bal);
				system("cls");
			}

			//-----------------------------------------------------------------------------------------------
			_itoa(bal, bal_buf, 10);
			send(s, bal_buf, strlen(bal_buf), 0);// 서버에서 클라이언트로 게임 인원수 user_set 부분을 보낸다.
			//-----------------------------------------------------------------------------------------------

			while (_getch != ENTER) {
				rewind(stdin);
				gotoxy(15, 15);
				printf("게임 설정이 완료되어 게임이 시작하기를 원하시면 Enter 키를 눌러주세요.");
			...
			MONEY_SHOW(user, user_set);
			Red1;								//제일 처음 플레이어 턴을 불러와서 출력할때 나오는 색깔 지정하는곳 (ex : RED1, GOLD1)

			gotoxy(23, 30); printf("                    　　　　      ");
			gotoxy(23, 30); printf("플레이어%d의 차례입니다.", TURN);
			gotoxy(23, 31); printf("                    　　　　      ");
			gotoxy(23, 31); printf("주사위를 굴리려면 ENTER를 입력해주세요.");

			

			while ((res = _getch()) != ENTER);
			_itoa(res, res_buf, 10);
			send(s, res_buf, strlen(res_buf), 0);

			gotoxy(23, 30); printf("                    　　　　      ");						//플레이어%d의 차례입니다. CLR
			gotoxy(23, 31); printf("                                  　　　　　　　           ");	//주사위를 굴리려면 ENTER를 입력해주세요. CLR
			STATE = P_TURN;
			break;
		}
		case INFO:
		{
			system("mode con cols=100 lines=33 | title 부르마블 Server");
			if (infor() == ESC)
				STATE = INTRO;
			else
				STATE = INFO;
		...
		case P_TURN:
		{
			...
			if (TURN == P2_TURN) {
				...
				if (stat->location->zone_name != "무인도") {
					if (stat->OE == EXIST) {
						nRcv = recv(s, selOE_buf, sizeof(selOE_buf) - 1, 0);
						if (nRcv == -1) {
							printf("값을 받지 못하였습니다.");
							_getch();
							system("cls");
				...
				//클라 주사위1
				nRcv = recv(s, dice1_buf, sizeof(dice1_buf) - 1, 0);
				if (nRcv == -1) {
					printf("값을 받지 못하였습니다.");
					_getch();
					system("cls");
				...
				nRcv = recv(s, dice2_buf, sizeof(dice2_buf) - 1, 0);
				if (nRcv == -1) {
					printf("값을 받지 못하였습니다.");
					_getch();
					system("cls");
				...
				if (stat->location->zone_name == "무인도") {
					...
					nRcv = recv(s, ENTER_buf, sizeof(ENTER_buf) - 1, 0);
					if (nRcv == -1) {
						printf("값을 받지 못하였습니다.");
						_getch();
						system("cls");
				...
				if (stat->location->zone_name != "무인도") {
					if (stat->OE == EXIST) {
						...
						send(s, selOE_buf, strlen(selOE_buf), 0);
				...
				send(s, dice1_buf, strlen(dice1_buf), 0);
				send(s, dice2_buf, strlen(dice2_buf), 0);
				...
				if (stat->location->zone_name == "무인도") {
					...
					send(s, ENTER_buf, strlen(ENTER_buf), 0);
		...
		case MINI_GAME:
		{
			...
			if (TURN == P1_TURN) {
				...
				send(s, use_buf, strlen(use_buf), 0);
				...
				nRcv = recv(s, use_buf, sizeof(use_buf) - 1, 0);
				if (nRcv == -1) {
					printf("값을 받지 못하였습니다.");
					_getch();
					system("cls");
...
;
			...
			if (TURN == P1_TURN) {
				while (1) {
					if (_kbhit()) {
						if ((pas = _getch()) == ENTER) {
							...
							send(s, result_buf, strlen(result_buf), 0);
							break;
						}
					}
					else {
				...
				nRcv = recv(s, result_buf, sizeof(result_buf) - 1, 0);
				if (nRcv == -1) {
					printf("값을 받지 못하였습니다.");
					_getch();
					system("cls");
			...
			if (use % 2 == result % 2) {
				...
				if (TURN == P1_TURN) {
					...
					send(s, sel_g_buf, strlen(sel_g_buf), 0);
				}
				else {
					nRcv = recv(s, sel_g_buf, sizeof(sel_g_buf) - 1, 0);
					if (nRcv == -1) {
						printf("값을 받지 못하였습니다.");
						_getch();
						system("cls");
		...
		case BEH_MASTERLESS:
		{
			...
			if (TURN == P2_TURN) {
				...
				nRcv = recv(s, sel_buf, sizeof(sel_buf) - 1, 0);
				...
				send(s, sel_buf, strlen(sel_buf), 0);
						...
						if (stat->money < stat->location->zone_cost) {
							...
							if (check == 2) {
								...
								if (TURN == P1_TURN) {
									...
									send(s, win_buf, strlen(win_buf), 0);
								}
								else {
									nRcv = recv(s, win_buf, sizeof(win_buf) - 1, 0);
									if (nRcv == -1) {
										printf("값을 받지 못하였습니다.");
										_getch();
										system("cls");
		...
		case ENOUGH_MONEY:
		{
			...
			if (stat->money >= SUM_VAL) {
				...
				if (TURN == P1_TURN) {
					...
					send(s, tmp_buf, strlen(tmp_buf), 0);
				}
				else {
					//0614
					nRcv = recv(s, tmp_buf, sizeof(tmp_buf) - 1, 0);
					if (nRcv == -1) {
						printf("값을 받지 못하였습니다.");
						_getch();
						system("cls");
		...
		case NOT_ENOUGH_MONEY:
		{
			...
			if (TURN == P2_TURN) {
				...
				nRcv = recv(s, zone_buf, sizeof(zone_buf) - 1, 0);
				if (nRcv == -1) {
					printf("값을 받지 못하였습니다.");
					_getch();
					system("cls");
				...
				{
					recv(s, STATE_buf, strlen(STATE_buf), 0);
				...
				else
				{
					...
					send(s, zone_buf, strlen(zone_buf), 0);
		...
		case BEH_MINE:
		{
			...
			if (TURN == P1_TURN) {
				...
				_itoa(SEL_BEH, sel_buf, 10);
				send(s, sel_buf, strlen(sel_buf), 0);			//10진수 문자열로된 sel_buf값을 전달 한다.
			}
			else {
				nRcv = recv(s, sel_buf, sizeof(sel_buf) - 1, 0);
				//gotoxy(65, 65); printf("　　　　　　　　　");
		...
		case GOLDEN_KEY:
		{
			...
			//take_golden_key(G_KEY, rand() % 24);
			//take_golden_key(G_KEY, 22); //디버깅할때 황금카드를 우주여행만 나오게 해줌.
			//pick = 22;
			if (TURN == P1_TURN) {
				//pick = 23;
				pick = rand() % 24;
				...
				send(s, key_buf, strlen(key_buf), 0);
				...
				nRcv = recv(s, key_buf, sizeof(key_buf) - 1, 0);
				if (nRcv == -1) {
					printf("값을 받지 못하였습니다.");
					_getch();
					system("cls");
		...
		case KEY12:
		{
			...
			//gotoxy(55, 25);	printf("ENTER를 입력하세요.");
			MONEY_SHOW(user, user_set);
			if (TURN == P1_TURN) {
				gotoxy(55, 25);	printf("ENTER를 입력해주세요.");
				while ((pas = _getch()) != ENTER);
		...
		case KEY24:
		{
			...
			if (TURN == P1_TURN) {
				...
				send(s, sel_p_buf, strlen(sel_p_buf), 0);
				...
				nRcv = recv(s, sel_p_buf, sizeof(sel_p_buf) - 1, 0);
				if (nRcv == -1) {
					printf("값을 받지 못하였습니다.");
					_getch();
					system("cls");
		...
		case GAME_OVER:
		{
			...
			system("mode con cols=100 lines=30 | title 부르마블 Server");

			if (COLOR_WIN != NULL) { //트리플독점 승리
				if (TURN == PLAYER1)
				{
					White1;
					gotoxy(31, 10);	printf("┏〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓┓\n");
